...
#definePFILE_NAME "myClock" 
#define SFOLDER_NAME "myClock2" // 
#defineSFILE_NAME "myClock2"   //filename same as folder for this example... 
#define SIG_TEST 44 
...
struct siginfo info; 
staticstruct task_struct *t[MAX_NUM_ALARMS]; 
...
//hw7 
static struct kmem_cache *tkp_alarm_struct_cache; 
static struct tkp_alarm_struct *this_alarm; 
 
struct tkp_alarm { 
    int delay; 
    struct list_head my_alarms; 
...
             struct tkp_alarm_struct *myalarm; 
 
             list_for_each(pos, &my_alarm_list) { 
             myalarm = list_entry(head, struct tkp_alarm_struct, my_alarm_list); 
             if(myalarm->delay == (int)data) { 
                 this_period = myalarm->delay; 
...
intret; 
int num; 
             this_alarm = kmem_cache_alloc(tkp_alarm_cache, GFP_KERNEL); 
...
{ 
//slab allocation 
tkp_alarm_struct_cache = kmem_cache_create("tkp_alarm", sizeof(struct tkp_alarm), 0, SLAB_HWCACHE_ALIGN, NULL); 
...
if(tkp_alarm_cache) { 
        kmem_cache_destroy(tkp_alarm_cache); 
    } 
...
if (ret) printk("Kobject_tkp: timer %d is still in use\n", i ); 
} 
printk("Kobject_tkp: uninstalling\n"); 
kobject_put(myClock2_kobj); 
} 
